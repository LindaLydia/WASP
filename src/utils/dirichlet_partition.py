...
# targets = np.array(dataset.targets)  # Labels of CIFAR-10 or any dataset
# # 3. Define the number of clients and alpha for Dirichlet distribution
# num_clients = 10
# alpha = 0.5  # Alpha value for Dirichlet distribution

# 4. Function to perform Dirichlet partitioning
def dirichlet_partition(args, targets, num_clients, alpha):
    num_classes = len(np.unique(targets))  # Number of classes in the dataset
    data_per_class = {i: np.where(targets == i)[0] for i in range(num_classes)}  # Indices for each class
...
def create_partitioned_dataset_and_dataloader(client_data_indices, dataset):
    # 5. Partition the dataset using the Dirichlet distribution
    client_data_indices = dirichlet_partition(targets, num_clients, alpha)
    # 6. Create DataLoader or Subset for each client based on partitioned indices
    client_datasets = {i: Subset(dataset, client_data_indices[i]) for i in range(num_clients)}
    # Example: Create DataLoader for the first client
    client_0_loaders = DataLoader(client_datasets[0], batch_size=32, shuffle=True)

    # You can now create DataLoaders for each client using their respective partitioned datasets.
