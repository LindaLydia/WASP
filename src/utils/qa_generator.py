...
class QADataGenerator:
    ...
    def generate_answer_ner(self, inputs) -> Dataset:
        # nlp = spacy.load("en_core_web_sm")
        nlp = spacy.load("../../../shared_models/en_core_web_sm-3.3.0/en_core_web_sm/en_core_web_sm-3.3.0/")
        # dataset = self.processor.dataset[self.processor.train_key]
        # columns = dataset.format['columns']
        dataset = pd.DataFrame(columns=['context'])
        # for i in range(len(inputs)):
        #     dataset.loc[len(dataset)] = inputs[i]
        new_rows = pd.DataFrame({'context': inputs, 'id': ['ID_'+str(_i) for _i in range(len(inputs))]})
        ...
        iter=new_dataset.iter(batch_size=1)
        # for i in iter:
        #     print(f'item is {i}')
        new_dataset = new_dataset.map(sample_ner, load_from_cache_file=False, num_proc=2)  # revise as needed

        examples = []
        for aug_examples in new_dataset['aug_examples']:
            examples.extend(aug_examples)

        new_dataset = Dataset.from_pandas(pd.DataFrame(examples, columns=['context','question', 'answers','id'])).shuffle(seed=self.seed,
