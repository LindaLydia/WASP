...
from transformers import AutoModelForSeq2SeqLM, AutoModelForCausalLM, AutoModelForMaskedLM, AutoTokenizer, AutoModelForQuestionAnswering
...
from utils.weight_adjust import weight_decay, weight_decay_qa, model_importance_estimation, model_importance_estimation_qa
...
def load_iters_bert(args, batch_size=32, backward_batch_size=1000, device="cpu", gold_data_path='data', syn_data_path='data', vectors=None, use_tree=False, num_use_samples_inner=100, num_use_samples_outer=100, shuffle_train=True):
    ...
    if args.gold_data == None:
        ...
        for i_gold in range(args.gold_party_num):
            indices = list(range(len(gold_data_list[i_gold].context)))
            random.shuffle(indices)
            gold_train_valid_pivot_point = int(len(gold_data_list[i_gold].context)*args.train_ratio)
            small_gold_train_data = TokenizedQADataset(
                file_path=(''),
            )
            small_gold_train_data.copy_dataset(gold_data_list[i_gold], indices[:gold_train_valid_pivot_point], new_idx=True)
            small_gold_valid_data = TokenizedQADataset(
                file_path=(''),
            )
    ...
    small_valid_iter_list = [DataLoader(dataset, batch_size=1, shuffle=shuffle_train) for dataset in small_valid_data_list]
    ...
    #         print(labels, idx)
    #         for _idx, _label in zip(idx, labels):
    #             print(f"model#{im}: idx={_idx}, label={small_train_data.label[_idx]}, text={small_train_data.context[_idx]}")
    #             # print(f"batch: idx={_idx}, batch_used_text={_text}")
    #         break
    ...
    if args.small_model_name.upper() == 'LSTM':
        with torch.no_grad():
            for i, batch in enumerate(data_iter):
                (inputs, lens), labels = batch.context, batch.label
        ...
        with torch.no_grad():
            for i, batch in enumerate(data_iter):
                ...
                # print(f"eval {inputs=}\n{attention_mask=}\n{offset_mapping=}\n{sample_mapping=}\n{idx=}")
                # print(f"before in <eval()>, {inputs.shape=}")
                inputs = inputs.squeeze(dim=0).to(args.device)
                # print(f"after in <eval()>, {inputs.shape=}")
                ...
                # start_positions, end_positions, answer_text = [[] for _ in range(len(labels))], [[] for _ in range(len(labels))], [[] for _ in range(len(labels))]
                # print(f"eval {labels=}")
                # for _i in len(labels):
                #     for _start_idx, _text in zip(labels["answer_start"][_i], list(labels["text"][_i])):
                #         for _ in range(len(inputs)):
                #             start_positions[_i].append(_start_idx.item())
                #             end_positions{_i}.append(_start_idx.item()+len(_text))
                #             answer_text[_i].append(_text)
                ...
                # print(f"eval {start_positions.shape=}, {end_positions.shape=}")
...
def train_to_converge(args, model, train_data, valid_data, theta, epoch_converge, inner_obj, test_loader, optimizer_state=None, soft_label=None):
    ...
    if args.small_model_name.upper() == 'LSTM':
        ...
        #     print(data.idx, data.context, data.label)
        train_iter, = BucketIterator.splits(
            (train_data,),
    ...
    if valid_data != None and epoch_converge>1:
        valid_iter = DataLoader(valid_data, batch_size=1, shuffle=False)
    ...
    for epoch in range(epoch_converge if (valid_data==None or epoch_converge==1) else int(epoch_converge*3)):
        ...
        for batch in tqdm(train_iter):
            ...
            eval_metrics = find_qa_predict_results(args, contexts, start_logits, end_logits, offset_mapping, sample_mapping, start_positions, answer_text, idx)
            f1 = eval_metrics['f1']
            top1.update(f1, labels["answer_start"][0].size(0))
...
def train_to_converge_with_weight_adjust_and_label_flip_fused(args, model, train_data, theta, trained_models, selected_sample_indexs, epoch_adjust, epoch_converge, inner_obj, test_loader, outer_iter):
    ...
    if args.small_model_name.upper() == 'LSTM':
        ...
        for row, column in zip(selected_sample_rows,selected_sample_columns):
            ...
            print(selected_train_dataset[_id].context, f"{row.item()=}, {column.item()=}")
        ...
        for _i in range(args.len_LLM):
            ...
            new_train_data[_i].copy_dataset(train_data[_i], [ix for ix in range(len(train_data[_i].context))], new_idx=False)
        change_count = 0
        for row, column in zip(selected_sample_rows,selected_sample_columns):
            _id = args.accumulate_sampels[row].item()+column
...
def train_to_converge_with_selection_kd_fused(args, model, train_data, theta, quality_separate_sample_indexs, soft_labels, epoch_adjust, epoch_converge, inner_obj, train_loader_backward, valid_loader, test_loader, outer_iter, final_test=True):
    ...
    if 'Reweight' in args.fuse_dataset_weight:
        ...
        if args.small_model_name.upper() == 'LSTM':
            ...
            selected_valid_data = TokenizedQADataset(
                file_path=(''),
            )
            ...
            train_iter = DataLoader(selected_train_data, batch_size=args.train_batch_size, shuffle=args.shuffle_train)
            valid_iter = DataLoader(selected_valid_data, batch_size=1, shuffle=False)
            print(f"{len(selected_train_data)=}, {theta.shape=}")
        # init_theta = copy.deepcopy(theta)
        for epoch in range(epoch_adjust//3):
...
def train_separate_models(args, model, train_data, small_train_data, small_valid_data, train_loader_backward, valid_loader, test_loader, main_outer_iter):
    ...
    total_valid_data = merge_all_dataset_qa(args, small_valid_data, max_sample_count_for_total=-1)
    ...
    for outer_iter in range(5):
        ...
        # model_total_acc = model_importance_estimation_qa(args, current_outer_iter_trained_more_steps_model, train_data, _type=args.weight_adjust_criterial)
...
def solve_with_local_cross_validation(args, model, train_data, small_train_data, small_valid_data, train_loader_backward, valid_loader, gold_loader, test_loader, perform_few_shot_gen=False):
    ...
    total_small_train_data = merge_all_dataset_qa(args, small_train_data, max_sample_count_for_total=-1)
    # print(f"check idx of dataset {total_small_train_data.idx=}")
    ...
    # for outer_iter in range(5):
        ...
        for _outer_iter in range(1 if 'Flip' in args.fuse_dataset_sample_selection else 1): # this is for separate model WA
                ...
                while diverged:
                    ...
                    if _outer_iter % args.check_ft_every==0:
                        ...
                        for im in range(args.len_LLM):
                            confidence_score[im], variability_score[im] = run_divergence_calculation_qa(args, model_weight_ckp_all_syn, small_train_data[im], plm_name=args.llms[im])
            ...
            if 'Adjust' in args.fuse_dataset_weight:
                theta_total_mapped, _depricated_model_total_acc = weight_decay_qa(args, [current_outer_iter_trained_model[-1]], [total_small_train_data], theta_total_mapped, beta=args.BETA, _type=args.weight_adjust_criterial, num_models=1)
                theta_total[0] = copy.deepcopy(theta_total_mapped[0])
                theta_total_score = copy.deepcopy(theta_total[0])
                best_theta_total[0]=theta_total_score
            # torch.save(tuple(current_outer_iter_trained_more_steps_model+[args.fused_model]), f"{args.result_file_path}/iter{args.i_step}_main_separate_models.pth")
        ...
        for i_party in range(args.gold_party_num):
            print(f"check idx of dataset {total_small_train_data.idx=}")
            mix_train_data = merge_all_dataset_qa(args, [total_small_train_data]+[gold_iter[i_party].dataset], max_sample_count_for_total=-1)
        ...
        # torch.save(([dataset.context for dataset in [total_small_train_data]], [dataset.question for dataset in [total_small_train_data]], [dataset.label for dataset in [total_small_train_data]], loss_per_sample_change, error_per_sample_change, correctness_per_sample_change, prediction_per_sample_change, norm_logits_per_sample_change), f"{args.result_file_path}/iter{args.i_step}_sample_pred_change.pth")
        ...
        if perform_few_shot_gen == True:
                ...
                if 'Flip' in args.fuse_dataset_sample_selection:
                    ...
                    if 'CartographyWithReal' in args.gen_sample_select:
                        if 'V2' in args.gen_sample_select:
                            for im in range(args.len_LLM):
                                confidence_score[im], variability_score[im] = run_divergence_calculation_qa(args, [current_outer_iter_trained_more_steps_model[-1]]+current_outer_iter_trained_more_steps_model_iter0_after_gold_v2, small_train_data[im], plm_name=args.llms[im])
                        else:
                            for im in range(args.len_LLM):
                                confidence_score[im], variability_score[im] = run_divergence_calculation_qa(args, [current_outer_iter_trained_more_steps_model[-1]]+current_outer_iter_trained_more_steps_model_iter0_after_gold, small_train_data[im], plm_name=args.llms[im])
                    elif not 'CartographyOriginal' in args.gen_sample_select:
                        for im in range(args.len_LLM):
                            confidence_score[im], variability_score[im] = run_divergence_calculation_qa(args, current_outer_iter_trained_more_steps_model, small_train_data[im], plm_name=args.llms[im])
                print(f"{confidence_score=}, {variability_score=}")
            ...
            if args.gen_by_class == 0: # different prompt for different class
                if 'Flip' in args.fuse_dataset_sample_selection:
                    ...
                    # only this brench should be reached
                    prompt_samples_idx, nearest_sample_voting, model_voting_score = find_nearest_syn_samples_multi_data_party_qa(args, small_train_data, gold_loader, args.gen_few_shot_k, sample_limitation=selected_indices)
            ...
            if args.gen_by_class == 0: # all labels share the same prompt
                if args.gen_aug_pe == 1:
                    ...
                    prompt_format_template = copy.deepcopy(prompt["instruction"])
                    ...
                    for i_sample in range(args.gen_few_shot_k):
                        ...
                        logging.info(f"prompt sample = {small_train_data[prompt_samples_idx[i_sample][0]].question[prompt_samples_idx[i_sample][1]]+' [SEP] '+small_train_data[prompt_samples_idx[i_sample][0]].context[prompt_samples_idx[i_sample][1]]}, label = {small_train_data[prompt_samples_idx[i_sample][0]].label[prompt_samples_idx[i_sample][1]]}")
                        few_shot_samples = (small_train_data[prompt_samples_idx[i_sample][0]].question[prompt_samples_idx[i_sample][1]]+" [SEP] "+small_train_data[prompt_samples_idx[i_sample][0]].context[prompt_samples_idx[i_sample][1]]).replace("{","").replace("}","")
                        prompt["instruction"].append(prompt_format_template.format(few_shot_samples))
                else:
                    prompt = copy.deepcopy(FEW_SHOT_PROMPT_WITH_GOOD_AND_BAD[args.task_name])
                    prompt_format_template = copy.deepcopy(prompt["instruction"])
                    prompt["instruction"] = []
                    for _j in range(6):
                        few_shot_samples = ''
                        ...
                        for i_sample in bad_sample_index:
                            print(f"{i_sample=}, {prompt_samples_idx['furthest'][i_sample][0]=}, {prompt_samples_idx['furthest'][i_sample][1]=}")
                            print(f"prompt sample = {small_train_data[prompt_samples_idx['furthest'][i_sample][0]].question[prompt_samples_idx['furthest'][i_sample][1]]+' [SEP] '+small_train_data[prompt_samples_idx['furthest'][i_sample][0]].context[prompt_samples_idx['furthest'][i_sample][1]]}, label = {small_train_data[prompt_samples_idx['furthest'][i_sample][0]].label[prompt_samples_idx['furthest'][i_sample][1]]}")
                            logging.info(f"{i_sample=}, {prompt_samples_idx['furthest'][i_sample][0]=}, {prompt_samples_idx['furthest'][i_sample][1]=}")
                            logging.info(f"prompt sample = {small_train_data[prompt_samples_idx['furthest'][i_sample][0]].question[prompt_samples_idx['furthest'][i_sample][1]]+' [SEP] '+small_train_data[prompt_samples_idx['furthest'][i_sample][0]].context[prompt_samples_idx['furthest'][i_sample][1]]}, label = {small_train_data[prompt_samples_idx['furthest'][i_sample][0]].label[prompt_samples_idx['furthest'][i_sample][1]]}")
                            few_shot_samples += f"{FEW_SHOT_SAMPLE_TEMPLATE_BAD[args.task_name]}{(small_train_data[prompt_samples_idx['furthest'][i_sample][0]].question[prompt_samples_idx['furthest'][i_sample][1]]+' [SEP] '+small_train_data[prompt_samples_idx['furthest'][i_sample][0]].context[prompt_samples_idx['furthest'][i_sample][1]]).replace('{','').replace('}','')}\n"
                        for i_sample in good_sample_index:
                            print(f"{i_sample=}, {prompt_samples_idx['nearest'][i_sample][0]=}, {prompt_samples_idx['nearest'][i_sample][1]=}")
                            print(f"prompt sample = {small_train_data[prompt_samples_idx['nearest'][i_sample][0]].question[prompt_samples_idx['nearest'][i_sample][1]]+' [SEP] '+small_train_data[prompt_samples_idx['nearest'][i_sample][0]].context[prompt_samples_idx['nearest'][i_sample][1]]}, label = {small_train_data[prompt_samples_idx['nearest'][i_sample][0]].label[prompt_samples_idx['nearest'][i_sample][1]]}")
                            logging.info(f"{i_sample=}, {prompt_samples_idx['nearest'][i_sample][0]=}, {prompt_samples_idx['nearest'][i_sample][1]=}")
                            logging.info(f"prompt sample = {small_train_data[prompt_samples_idx['nearest'][i_sample][0]].question[prompt_samples_idx['nearest'][i_sample][1]]+' [SEP] '+small_train_data[prompt_samples_idx['nearest'][i_sample][0]].context[prompt_samples_idx['nearest'][i_sample][1]]}, label = {small_train_data[prompt_samples_idx['nearest'][i_sample][0]].label[prompt_samples_idx['nearest'][i_sample][1]]}")
                            few_shot_samples += f"{FEW_SHOT_SAMPLE_TEMPLATE_GOOD[args.task_name]}{(small_train_data[prompt_samples_idx['nearest'][i_sample][0]].question[prompt_samples_idx['nearest'][i_sample][1]]+' [SEP] '+small_train_data[prompt_samples_idx['nearest'][i_sample][0]].context[prompt_samples_idx['nearest'][i_sample][1]]).replace('{','').replace('}','')}\n"
                ...
                if 'Contrast' in args.gen_sample_select:
                    ...
                    for i_key, key in enumerate(prompt["labels"].keys()):
                        ...
                        for _j in range(6):
                            ...
                            for i_sample in bad_sample_index:
                                ...
                                print(f"prompt sample = {small_train_data[prompt_samples_idx['furthest'][i_key][i_sample][0]].question[prompt_samples_idx['furthest'][i_key][i_sample][1]]+' [SEP] '+small_train_data[prompt_samples_idx['furthest'][i_key][i_sample][0]].context[prompt_samples_idx['furthest'][i_key][i_sample][1]]}, label = {small_train_data[prompt_samples_idx['furthest'][i_key][i_sample][0]].label[prompt_samples_idx['furthest'][i_key][i_sample][1]]}")
                                logging.info(f"{i_sample=}, {prompt_samples_idx['furthest'][i_key][i_sample][0]=}, {prompt_samples_idx['furthest'][i_key][i_sample][1]=}")
                                logging.info(f"prompt sample = {small_train_data[prompt_samples_idx['furthest'][i_key][i_sample][0]].question[prompt_samples_idx['furthest'][i_key][i_sample][1]]+' [SEP] '+small_train_data[prompt_samples_idx['furthest'][i_key][i_sample][0]].context[prompt_samples_idx['furthest'][i_key][i_sample][1]]}, label = {small_train_data[prompt_samples_idx['furthest'][i_key][i_sample][0]].label[prompt_samples_idx['furthest'][i_key][i_sample][1]]}")
                                few_shot_samples += f"{FEW_SHOT_SAMPLE_TEMPLATE_BAD[args.task_name]}{(small_train_data[prompt_samples_idx['furthest'][i_key][i_sample][0]].question[prompt_samples_idx['furthest'][i_key][i_sample][1]]+' [SEP] '+small_train_data[prompt_samples_idx['furthest'][i_key][i_sample][0]].context[prompt_samples_idx['furthest'][i_key][i_sample][1]]).replace('{','').replace('}','')}\n"
                            for i_sample in good_sample_index:
                                print(f"{key=}, {i_key=} {i_sample=}")
                                print(f"{i_sample=}, {prompt_samples_idx['nearest'][i_key][i_sample][0]=}, {prompt_samples_idx['nearest'][i_key][i_sample][1]=}")
                                print(f"prompt sample = {small_train_data[prompt_samples_idx['nearest'][i_key][i_sample][0]].question[prompt_samples_idx['nearest'][i_key][i_sample][1]]+' [SEP] '+small_train_data[prompt_samples_idx['nearest'][i_key][i_sample][0]].context[prompt_samples_idx['nearest'][i_key][i_sample][1]]}, label = {small_train_data[prompt_samples_idx['nearest'][i_key][i_sample][0]].label[prompt_samples_idx['nearest'][i_key][i_sample][1]]}")
                                logging.info(f"{i_sample=}, {prompt_samples_idx['nearest'][i_key][i_sample][0]=}, {prompt_samples_idx['nearest'][i_key][i_sample][1]=}")
                                logging.info(f"prompt sample = {small_train_data[prompt_samples_idx['nearest'][i_key][i_sample][0]].question[prompt_samples_idx['nearest'][i_key][i_sample][1]]+' [SEP] '+small_train_data[prompt_samples_idx['nearest'][i_key][i_sample][0]].context[prompt_samples_idx['nearest'][i_key][i_sample][1]]}, label = {small_train_data[prompt_samples_idx['nearest'][i_key][i_sample][0]].label[prompt_samples_idx['nearest'][i_key][i_sample][1]]}")
                                few_shot_samples += f"{FEW_SHOT_SAMPLE_TEMPLATE_GOOD[args.task_name]}{(small_train_data[prompt_samples_idx['nearest'][i_key][i_sample][0]].question[prompt_samples_idx['nearest'][i_key][i_sample][1]]+' [SEP] '+small_train_data[prompt_samples_idx['nearest'][i_key][i_sample][0]].context[prompt_samples_idx['nearest'][i_key][i_sample][1]]).replace('{','').replace('}','')}\n"
                            print(f"{type(prompt)=}, {type(prompt_format_template)=}")
                            print(f"{prompt=}, {prompt_format_template=}")
                            print(f"{prompt_format_template.format(few_shot_samples, '{}')=}")
                    ...
                    for i_key, key in enumerate(prompt["labels"].keys()):
                        ...
                        for i_sample in range(args.gen_few_shot_k):
                            ...
                            logging.info(f"prompt sample = {small_train_data[prompt_samples_idx[i_key][i_sample][0]].question[prompt_samples_idx[i_key][i_sample][1]]+' [SEP] '+small_train_data[prompt_samples_idx[i_key][i_sample][0]].context[prompt_samples_idx[i_key][i_sample][1]]}, label = {small_train_data[prompt_samples_idx[i_key][i_sample][0]].label[prompt_samples_idx[i_key][i_sample][1]]}")
                            few_shot_samples = (small_train_data[prompt_samples_idx[i_key][i_sample][0]].question[prompt_samples_idx[i_key][i_sample][1]]+" [SEP] "+small_train_data[prompt_samples_idx[i_key][i_sample][0]].context[prompt_samples_idx[i_key][i_sample][1]]).replace("{","").replace("}","")
                            prompt["labels"][key]["instruction"].append(prompt_format_template.format(few_shot_samples))
                    ...
                    for i_key, key in enumerate(prompt["labels"].keys()):
                        ...
                        for i_sample in range(args.gen_few_shot_k):
                            ...
                            logging.info(f"{i_sample=}, {prompt_samples_idx[i_key][i_sample][0]=}, {prompt_samples_idx[i_key][i_sample][1]=}")
                            logging.info(f"prompt sample = {small_train_data[prompt_samples_idx[i_key][i_sample][0]].question[prompt_samples_idx[i_key][i_sample][1]]+' [SEP] '+small_train_data[prompt_samples_idx[i_key][i_sample][0]].context[prompt_samples_idx[i_key][i_sample][1]]}, label = {small_train_data[prompt_samples_idx[i_key][i_sample][0]].label[prompt_samples_idx[i_key][i_sample][1]]}")
                            few_shot_samples += f'{FEW_SHOT_SAMPLE_TEMPLATE[args.task_name]}{(small_train_data[prompt_samples_idx[i_key][i_sample][0]].question[prompt_samples_idx[i_key][i_sample][1]]+" [SEP] "+small_train_data[prompt_samples_idx[i_key][i_sample][0]].context[prompt_samples_idx[i_key][i_sample][1]]).replace("{","").replace("}","")}\n'
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_ratio", type=float, default=0.01, help="separate a ratio of syn-dataset for model importance estimation")
            ...
            if args.gold_iter == None:
                ...
                for i_data_party in range(args.gold_party_num):
                    ...
                    for sample_file_name in ['train_noflip', 'train']:
                        save_gold_sample_file_qa(f'{args.working_gold_sample_dir[i_data_party]}{sample_file_name}.jsonl', args.gold_data[i_data_party])
        else: # lstm
