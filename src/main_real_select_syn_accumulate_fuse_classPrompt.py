...
def solve_with_local_cross_validation(args, model, train_data, small_train_data, small_valid_data, train_loader_backward, valid_loader, gold_loader, test_loader, perform_few_shot_gen=False):
    ...
    # for outer_iter in range(5):
        ...
        if perform_few_shot_gen == True:
            ...
            if 'Flip' in args.fuse_dataset_sample_selection:
                prompt_samples_idx, nearest_sample_voting, model_voting_score = find_nearest_syn_samples_byClass(args, new_small_train_data, gold_loader, args.gen_few_shot_k, sample_limitation=selected_indices)
            else:
                prompt_samples_idx, nearest_sample_voting, model_voting_score = find_nearest_syn_samples_byClass(args, small_train_data, gold_loader, args.gen_few_shot_k, sample_limitation=selected_indices)
            print(f'here0-6(1), {torch.cuda.memory_reserved()/1024/1024=}M, {torch.cuda.memory_allocated()/1024/1024=}M')
            print(f"{prompt_samples_idx=}") # prompt_samples_idx = [[(1,0),(1,1),(0,2),(0,3)], [(1,1),(0,5),(1,0),(0,1)]] each for a specific class
...
if __name__ == "__main__":
    ...
    if args.steps == 0:
        ...
        for im in range(args.len_LLM):
            ...
            args.working_sample_dir.append(f'{SYN_DATA_PATH}voting{args.voting_range.upper()}_{args.real_voting_votes}_{args.gen_sample_select}_{args.voted_sample_select}_ClassPrompt/{args.model_name_sample}/{args.small_model_name}/{args.sentence_transformer}/{args.fuse_dataset_sample_selection}_KD{args.kd_slm}_FuseDataset{args.fuse_dataset}/fewshotK{args.gen_few_shot_k}_{args.gen_few_shot_pool_size}_{args.gen_few_shot_ambiguous_ratio}/{args.seed}/{args.llms[im]}/{args.num_use_samples_inner[im]}_{args.num_use_samples_init[im]}_{args.steps}_{"un" if args.unbalance_generation else ""}balance_temp{args.unbalance_generation_temperature}/')
            ...
            args.working_prompt_dir.append(f'{SYN_DATA_PATH}voting{args.voting_range.upper()}_{args.real_voting_votes}_{args.gen_sample_select}_{args.voted_sample_select}_ClassPrompt/{args.model_name_sample}/{args.small_model_name}/{args.sentence_transformer}/{args.fuse_dataset_sample_selection}_KD{args.kd_slm}_FuseDataset{args.fuse_dataset}/fewshotK{args.gen_few_shot_k}_{args.gen_few_shot_pool_size}_{args.gen_few_shot_ambiguous_ratio}/{args.seed}/prompt/{args.llms[im]}/{args.num_use_samples_inner[im]}_{args.num_use_samples_init[im]}_{args.steps}_{"un" if args.unbalance_generation else ""}balance_temp{args.unbalance_generation_temperature}/')
