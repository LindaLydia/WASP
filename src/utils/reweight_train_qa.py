...
def construct_outer_subloader_qa(args, train_data, indices = None, idx_to_order=None):
    if indices is None:
...
def construct_outer_loader_qa(args, train_data, indices = None, idx_to_order=None):
...
def assign_learning_rate_qa(optimizer, new_lr):
    for param_group in optimizer.param_groups:
        param_group["lr"] = new_lr


def get_grad_weights_on_valid_qa(args, model, val_iter, reweight_theta, soft_label=None):
    soft_label == None
...
def repass_backward_qa(args, model, model_checkpoints, opt_checkpoints, outer_grads_w, train_data, reweight_theta_mapped, reweight_theta, soft_label=None):
    ...
    with torch.backends.cudnn.flags(enabled=False):
        for batch_idx, batch in enumerate(train_iter):
            ...
            if args.small_model_name.upper() == 'LSTM':
                old_params_, w_mapped = pseudo_updated_params_qa(args, model_copy, old_params, old_opt, inputs, lens, labels, reweight_theta_mapped[idx], reweight_theta_sum)
            elif 'bert' in args.small_model_name.lower() or 'ernie' in args.small_model_name.lower():
                old_params_, w_mapped = pseudo_updated_params_bert_qa(args, model_copy, old_params, old_opt, inputs, attention_mask, labels, reweight_theta_mapped[idx], reweight_theta_sum)
...
def pseudo_updated_params_qa(args, pseudo_net, model_checkpoint, opt_checkpoint, inputs, lens, labels, reweight_theta, reweight_theta_sum):
...
def pseudo_updated_params_bert_qa(args, pseudo_net, model_checkpoint, opt_checkpoint, inputs, attention_mask, labels, reweight_theta, reweight_theta_sum):
    pseudo_optimizer = MetaSGD(pseudo_net, pseudo_net.parameters(), lr=args.inner_lr)
    w_old = [p for p in pseudo_net.parameters()]
...
def eval_for_reweight_qa(args, model, data_iter, name, epoch=None):
    model.to(args.device)
    model.eval()
    correct_num = 0
    err_num = 0
...
def train_to_converge_reweight_qa(args, model, train_data, reweight_theta, epoch_converge, inner_obj, soft_label=None):
...
def solve_reweight_qa(args, model, train_data, theta, selected_sample_indexs, train_loader_backward, valid_loader, test_loader, soft_label=None):
    ...
    for outer_iter in range(args.max_reweight_outer_iter):
        ...
        if not args.disable_outer_scheduler:
            assign_learning_rate_qa(reweight_theta_opt, 0.5 * (1 + np.cos(np.pi * outer_iter / args.max_reweight_outer_iter)) * args.outer_lr)
        diverged = True # diverged==True means loss==nan, which means the training failed
        while diverged:
            model_copy_converged, loss, train_acc, model_weights_cache, opt_checkpoints_cache, diverged = train_to_converge_reweight_qa(args, model, selected_train_data, reweight_theta_mapped.detach(),args.epoch_converge,args.inner_obj, soft_label=soft_label)
            print(f"diverged {diverged} loss {loss}")
            print(f'train acc{train_acc}')
            if outer_iter % args.check_ft_every==0:
                model_copy_converged_ft, loss_ft, train_acc_ft, _, _, _ = train_to_converge_reweight_qa(args, model, selected_train_data, reweight_theta_mapped.detach(), args.epoch_converge_fully_train,args.inner_obj, soft_label=soft_label)
        ...
        valid_loader = construct_outer_loader_qa(args, selected_train_data)
        valid_soft_label = soft_label
        grad_weights_on_full_train, top1_outer, loss_outer = get_grad_weights_on_valid_qa(args, model_copy_converged, valid_loader, reweight_theta_mapped.detach(), soft_label=valid_soft_label)
        print(f"outer acc {top1_outer}, loss_outer {loss_outer}")
        grad_reweight_theta = repass_backward_qa(args, model, model_weights_cache, opt_checkpoints_cache, grad_weights_on_full_train, selected_train_data, reweight_theta_mapped, reweight_theta, soft_label=soft_label)
        ...
        if outer_iter % args.check_ft_every == 0:
            test_acc1_ft, test_loss_ft = eval_for_reweight_qa(args, model_copy_converged_ft, test_loader, name="test")
...
def solve_reweight_v2_qa(args, model, train_data, theta, selected_sample_indexs, train_loader_backward, valid_loader, test_loader, print_info, reweight_epoch, soft_label=None):
